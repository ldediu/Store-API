...
const show_products_in_order = async (
  req: express.Request,
...
const add_products_to_order = async (
  req: express.Request,
  res: express.Response
) => {
...
const update_products_in_order = async (
  req: express.Request,
  res: express.Response
) => {
  ...
  try {
    ...
    const result_order = await orderDetStore.update_products(
      req.params.id,
      updated_order
    );
    res.status(200);
...
const delete_all_products_from_order = async (
  req: express.Request,
  res: express.Response
) => {
  if (!req.params.id) {
...
const orderDetailedRoutes = (app: express.Application): void => {
  ...
  app.delete(
    "/orders/:id/products",
    verifyAuthToken,
    delete_all_products_from_order
  );
};
